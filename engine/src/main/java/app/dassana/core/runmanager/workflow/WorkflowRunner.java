...
import app.dassana.core.workflowmanager.risk.eval.RiskEvalException;
import app.dassana.core.workflowmanager.risk.eval.RiskEvalRequest;
import app.dassana.core.workflowmanager.risk.eval.RiskEvaluator;
...
import app.dassana.core.workflowmanager.workflow.model.Component;
...
import app.dassana.core.workflowmanager.workflow.model.WorkflowOutput;
import app.dassana.core.workflowmanager.workflow.model.WorkflowOutputException;
...
public class WorkflowRunner {
  ...
  private final static String[] requiredNormalizedFields = new String[]{"vendorId", "alertId", "csp", "resourceContainer"};
    ...
    if (matchingWorkflow.isPresent()) {
      ...
      for (Step step : workflowSteps) {
        ...
        try {
          ...
          // checks to see if the step is resource-info ( this step populates the required fields)
          if (step.getId().contentEquals("resource-info")) {
            // set to keep track of the missing fields
            Set<String> missingRequire = new HashSet<>();
            for (String s : requiredNormalizedFields) {
              // check for required fields
              if (!stepOutputJson.contains(s)) {
                missingRequire.add(s);
              }
            }

            if (!missingRequire.isEmpty()) {
              // we can change this to throw an error and halt the process. for now, we can just show an error message
              // and continue with the process.
              Error error = new Error(workflow.getId(), workflow.getType(), Component.STEP, step.getId(),
                      new Message("Following required fields are missing from normalize workflow output: " +
                              missingRequire + ". Please ensure these fields are present.", Severity.ERROR));
              errorList.add(error);
            }
          }
