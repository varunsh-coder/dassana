...
This is the first workflow which gets run and its output is sent to all of the workflows which have type `general-context`, `resource-context` and `policy-context` (don't worry, not all of then will run, only the ones which match the filters defined in those workflows will run).

## Example

Here is an example of how AWS security hub alerts are normalized.
...
Used for bidirectional sync for certain vendors (ie SecurityHub).
...
If set to true, alerts will be sent to the outbound queue. A use case for not sending alerts to the outbound queue is when you have a bidirectional sync with a specific vendor (see [`post-processor`](#post-processor) above).
