...
from dassana.common.models import NormalizedOutput, ResourceHierarchy
    ...
    with open(file_path, 'r') as f:
        ...
        for fil in filter(lambda x: x.get('vendor', '') == vendor, filters):
            if policyId in fil.get('policies', []):
                return yml.get('service'), yml.get('resource-type'), yml.get('csp'), ResourceHierarchy(
...
def handle(event: Input, context: LambdaContext):
    ...
    event.input.resourceHierarchy = rh if rh is not None else event.input.resourceHierarchy
    ### Pydantic bug with serialization requires some correction
    event.input.resourceHierarchy.classRh = class_ if class_ is not None else event.input.resourceHierarchy.classRh
